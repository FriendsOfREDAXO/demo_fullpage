...
var fps_settings = {
    ...
    afterRender: function () {
        ...
        $('.pagewrapper').animate({ opacity: 1 }, 400, function () { // Animation complete.
            if (!fpIsResponsive()) {
                $('.mainnav').addClass('uk-animation-fade').show();
                $('#fp-nav').addClass('uk-animation-shake').fadeIn(300); // Navigation-Dots einblenden
            } else {
                $('.hamburger').fadeIn(300);
            }
            // Logo einblenden
            $('.pagelogo img').addClass('uk-animation-fade').show();
            ...
            $('.section-idx-1').find('h1').addClass('uk-animation-slide-left').addClass('uk-animation-fade').show();
            // Animation Image beim 1. Slide
            $('.section-idx-1').find('.full').addClass('uk-animation-slide-right').addClass('uk-animation-fade').show();
            ...
            // Body-Background setzen wg. Easing-Effekt
    ...
    afterLoad: function (anchorLink, index) {
        ...
        // Navidots hell/dunkel
        slidemode = index % 2;
        if (slidemode == 1) {
            $('.section-idx-' + index).find('.fp-slidesNav').addClass('dark');
            $('#fp-nav').addClass('dark');
        } else {
            $('.section-idx-' + index).find('.fp-slidesNav').removeClass('dark');
    ...
    onLeave: function (index, nextIndex, direction) {
        ...
        // Navidots hell/dunkel
        slidemode = nextIndex % 2;
        if (slidemode == 1) {
            $('#fp-nav').addClass('dark');
            $('.section-idx-' + nextIndex).find('.fp-slidesNav').addClass('dark');
        } else {
            $('#fp-nav').removeClass('dark');
            $('.section-idx-' + nextIndex).find('.fp-slidesNav').removeClass('dark');
        }

        // Switch Logo + Navigation
        if (!fpIsResponsive()) {
...
$(document).ready(function () {
    ...
    $('.hamburger').on('click', function (e) {
        ...
        if ($(this).hasClass('is-active') === true) {
            $('.mainnav').slideUp(300).removeClass('active');
            $('div.pagelogo').removeClass('active');
        } else {
            $('.mainnav').slideDown(300, function () {
                $('.mainnav').addClass('active');
                $('div.pagelogo').addClass('active');
            });
        }
